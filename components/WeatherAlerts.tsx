...
const WeatherAlerts: React.FC<Props> = ({ alerts }) => {
  ...
  const bgColor = isSevere ? 'bg-red-100 dark:bg-red-900/30' : 'bg-orange-100 dark:bg-orange-900/30';
  const textColor = isSevere ? 'text-red-900 dark:text-red-100' : 'text-orange-900 dark:text-orange-100';
  const iconColor = isSevere ? 'text-red-600 dark:text-red-400' : 'text-orange-600 dark:text-orange-400';

  return (
    <div className={`w-full rounded-[2rem] p-4 ${bgColor} transition-all duration-300 animate-fade-in border border-transparent dark:border-white/5`}>
      <div
        className="flex items-start justify-between cursor-pointer"
        onClick={() => setExpanded(!expanded)}
      >
        <div className="flex gap-3">
          <div className={`p-2 bg-white/60 dark:bg-black/20 rounded-full ${iconColor}`}>
